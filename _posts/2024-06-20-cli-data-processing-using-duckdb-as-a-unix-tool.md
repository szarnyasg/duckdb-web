...
Joining the two tables together using `POSITIONAL JOIN` results in two `city` columns – we use the [`EXCLUDE` modifier]({% link docs/sql/expressions/star.md %}#exclude-modifier) to remove the duplicate column:
